    ...
    for (int i = minCoord.x(); i <= maxCoord.x(); ++i) {
        ...
        for (int j = minCoord.y(); j <= maxCoord.y(); ++j) {
            for (int k = minCoord.z(); k <= maxCoord.z(); ++k) {
                ...
                //float intrSigma = Lerp((closestPoint.z() - minZ) * invZInterval, sigma, minSigma);
                float intrAlpha = alpha;
                //float intrAlpha = Lerp((closestPoint.y() - minY) * invYInterval, minAlpha, alpha);
