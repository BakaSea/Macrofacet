...
int main(int argc, char* argv[]) {
    ...
    nanovdb::GridBuilder<float> builder = nanovdb::GridBuilder(0.f);
    auto& accessor = builder.getAccessor();
    auto handle = builder.getHandle(dx, nanovdb::Vec3d(0), "density");
    ...
    for (int i = minCoord.x(); i <= maxCoord.x(); ++i) {
        ...
        for (int j = minCoord.y(); j <= maxCoord.y(); ++j) {
            for (int k = minCoord.z(); k <= maxCoord.z(); ++k) {
                ...
                float intrSigma =
                    Lerp((closestPoint.x() - minX) * invXInterval, minSigma, sigma);
                if (-3.f * intrSigma < dist && dist < 3.f * intrSigma) {
                    float d = Density(dist, intrSigma, 1.f);
                    accessor.setValue(nanovdb::Coord(i, j, k), d);
                }
            }
        }
    }

    handle = builder.getHandle(dx, nanovdb::Vec3d(0), "density");
